...
library(ggthemes)
library(ggpubr)
plot_boxplot= function(df,x,y, test.method,ylab, title){
  if(is.null(df)){
    print("Usage is plot_violin(df,x,y, ylab, ggttle)")
  }else{
    p=ggboxplot(df, x=x, y=y, fill = x, size =0.5, alpha = 0.5)+
      geom_jitter(position=position_jitter(width=.1, height=0), size = 0.5)+#avoid plotting outliers twice
      stat_compare_means(method = test.method, paired = F, size=12,vjust = 0.5, label = "p.signif")+
